...
class Evaluator:
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            default=["pha_mad", "pha_mse", "pha_grad", "pha_dtssd", "fgr_mse"],
        )
    ...
    def evaluate_worker(self, dataset, clip, position):
        ...
        for i, framename in enumerate(
            ...
            true_pha = (
                torch.from_numpy(true_pha).cuda(non_blocking=True).float().div_(255)
            )
            pred_pha = (
                torch.from_numpy(pred_pha).cuda(non_blocking=True).float().div_(255)
            )
            ...
            if "fgr_mse" in self.args.metrics:
                true_fgr = cv2.imread(
                    os.path.join(self.args.true_dir, dataset, clip, "fgr", framename),
                    cv2.IMREAD_COLOR,
                )
                pred_fgr = cv2.imread(
                    os.path.join(self.args.pred_dir, dataset, clip, "fgr", framename),
                    cv2.IMREAD_COLOR,
                )
...
class MetricGRAD:
    ...
    def gauss_gradient(self, img):
        img_filtered_x = kornia.filters.filter2D(
            img[None, None, :, :], self.filter_x, border_type="replicate"
        )[0, 0]
        img_filtered_y = kornia.filters.filter2D(
            img[None, None, :, :], self.filter_y, border_type="replicate"
        )[0, 0]
