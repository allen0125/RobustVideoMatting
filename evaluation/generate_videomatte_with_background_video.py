...
videomatte_filenames = [
    (clipname, sorted(os.listdir(os.path.join(args.videomatte_dir, "fgr", clipname))))
...
for i in range(args.num_samples):
    bgrs = pims.PyAVVideoReader(
        os.path.join(
            args.background_dir, background_filenames[i % len(background_filenames)]
        )
    )
    ...
    for t in tqdm(range(args.num_frames), desc=str(i).zfill(4)):
        with Image.open(
            os.path.join(args.videomatte_dir, "fgr", clipname, framenames[base_t + t])
        ) as fgr, Image.open(
            os.path.join(args.videomatte_dir, "pha", clipname, framenames[base_t + t])
        ) as pha:
            fgr = fgr.convert("RGB")
            pha = pha.convert("L")
        ...
        com = Image.fromarray(
            np.uint8(np.asarray(fgr) * pha + np.asarray(bgr) * (1 - pha))
        )
