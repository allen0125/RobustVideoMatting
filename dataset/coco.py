...
class CocoPanopticDataset(Dataset):
    def __init__(self, imgdir: str, anndir: str, annfile: str, transform=None):
        with open(annfile) as f:
            self.data = json.load(f)["annotations"]
            self.data = list(
                filter(
                    lambda data: any(
                        info["category_id"] == 1 for info in data["segments_info"]
                    ),
                    self.data,
                )
            )
    ...
    def _load_img(self, data):
        with Image.open(
            os.path.join(self.imgdir, data["file_name"].replace(".png", ".jpg"))
        ) as img:
...
class CocoPanopticTrainAugmentation:
    ...
    def __call__(self, img, seg):
        ...
        params = transforms.RandomResizedCrop.get_params(
            img, scale=(0.5, 1), ratio=(0.7, 1.3)
        )
        img = F.resized_crop(
            img, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
        seg = F.resized_crop(
            seg, *params, self.size, interpolation=F.InterpolationMode.NEAREST
        )
