...
class MotionAugmentation:
    ...
    def __call__(self, fgrs, phas, bgrs):
        ...
        fgrs = F.resized_crop(
            fgrs, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
        phas = F.resized_crop(
            phas, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
        params = transforms.RandomResizedCrop.get_params(
            bgrs, scale=(1, 1), ratio=self.aspect_ratio_range
        )
        bgrs = F.resized_crop(
            bgrs, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
    ...
    def _motion_affine(self, *imgs):
        ...
        (
            ...
            scaleA,
            (shearXA, shearYA),
        ) = transforms.RandomAffine.get_params(**config)
        (
            angleB,
            (transXB, transYB),
            scaleB,
    ...
    def _motion_color_jitter(self, *imgs):
        ...
        for t in range(T):
            ...
            for img in imgs:
                img[t] = F.adjust_brightness(
                    img[t], max(1 + lerp(brightnessA, brightnessB, percentage), 0.1)
                )
                img[t] = F.adjust_contrast(
                    img[t], max(1 + lerp(contrastA, contrastB, percentage), 0.1)
                )
                img[t] = F.adjust_saturation(
                    img[t], max(1 + lerp(brightnessA, brightnessB, percentage), 0.1)
                )
                img[t] = F.adjust_hue(
                    img[t], min(0.5, max(-0.5, lerp(hueA, hueB, percentage) * 0.1))
                )
        return imgs if len(imgs) > 1 else imgs[0]
