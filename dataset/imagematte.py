...
class ImageMatteDataset(Dataset):
    ...
    def __len__(self):
        return max(
            len(self.imagematte_files),
            len(self.background_image_files) + len(self.background_video_clips),
        )
    ...
    def _get_imagematte(self, idx):
        with Image.open(
            os.path.join(
                self.imagematte_dir,
                "fgr",
                self.imagematte_files[idx % len(self.imagematte_files)],
            )
        ) as fgr, Image.open(
            os.path.join(
                self.imagematte_dir,
        ...
        with Image.open(
            os.path.join(
                ...
                self.background_image_files[
                    random.choice(range(len(self.background_image_files)))
                ],
            )
        ) as bgr:
