...
class YouTubeVISDataset(Dataset):
    def __init__(
        self, videodir, annfile, size, seq_length, seq_sampler, transform=None
    ):
        ...
        for ann in data["annotations"]:
            if ann["category_id"] == 26:  # person
                ...
                if video_id not in self.masks:
                    self.masks[video_id] = [
                        [] for _ in range(len(ann["segmentations"]))
                    ]
    ...
    def __getitem__(self, idx):
        ...
        for t in self.seq_sampler(self.seq_length):
            ...
            with Image.open(os.path.join(self.videodir, filename)) as img:
                imgs.append(
                    self._downsample_if_needed(img.convert("RGB"), Image.BILINEAR)
                )
...
class YouTubeVISAugmentation:
    ...
    def __call__(self, imgs, segs):
        ...
        params = transforms.RandomResizedCrop.get_params(
            imgs, scale=(0.8, 1), ratio=(0.9, 1.1)
        )
        imgs = F.resized_crop(
            imgs, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
        segs = F.resized_crop(
            segs, *params, self.size, interpolation=F.InterpolationMode.BILINEAR
        )
