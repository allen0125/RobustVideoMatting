...
class Trainer:
    ...
    def parse_args(self):
        ...
        # Matting dataset
        parser.add_argument(
            "--dataset", type=str, required=True, choices=["videomatte", "imagematte"]
        )
        # Learning rate
    ...
    def init_datasets(self):
        ...
        if self.args.dataset == "videomatte":
            ...
            self.dataset_valid = VideoMatteDataset(
                ...
                size=self.args.resolution_hr
                if self.args.train_hr
                else self.args.resolution_lr,
                seq_length=self.args.seq_length_hr
                if self.args.train_hr
                else self.args.seq_length_lr,
                seq_sampler=ValidFrameSampler(),
                transform=VideoMatteValidAugmentation(
                    size_hr if self.args.train_hr else size_lr
                ),
            )
        else:
            ...
            self.dataset_valid = ImageMatteDataset(
                ...
                seq_sampler=ValidFrameSampler(),
                transform=ImageMatteAugmentation(
                    size_hr if self.args.train_hr else size_lr
                ),
            )
    ...
    def init_model(self):
        ...
        self.model = MattingNetwork(
            self.args.model_variant, pretrained_backbone=True
        ).to(self.rank)
    ...
    def train(self):
        for epoch in range(self.args.epoch_start, self.args.epoch_end):
            ...
            for true_fgr, true_pha, true_bgr in tqdm(
                self.dataloader_lr_train,
                ...
                if self.step % 2 == 0:
                    ...
                    self.train_seg(
                        true_img.unsqueeze(1),
                        true_seg.unsqueeze(1),
                        log_label="seg_image",
                    )
    ...
    def train_mat(self, true_fgr, true_pha, true_bgr, downsample_ratio, tag):
        ...
        with autocast(enabled=not self.args.disable_mixed_precision):
            pred_fgr, pred_pha = self.model_ddp(
                true_src, downsample_ratio=downsample_ratio
            )[:2]
        ...
        if self.rank == 0 and self.step % self.args.log_train_images_interval == 0:
            self.writer.add_image(
                f"train_{tag}_pred_fgr",
                make_grid(pred_fgr.flatten(0, 1), nrow=pred_fgr.size(1)),
                self.step,
            )
            self.writer.add_image(
                f"train_{tag}_pred_pha",
                make_grid(pred_pha.flatten(0, 1), nrow=pred_pha.size(1)),
                self.step,
            )
            self.writer.add_image(
                f"train_{tag}_true_fgr",
                make_grid(true_fgr.flatten(0, 1), nrow=true_fgr.size(1)),
                self.step,
            )
            self.writer.add_image(
                f"train_{tag}_true_pha",
                make_grid(true_pha.flatten(0, 1), nrow=true_pha.size(1)),
                self.step,
            )
            self.writer.add_image(
                f"train_{tag}_true_src",
                make_grid(true_src.flatten(0, 1), nrow=true_src.size(1)),
                self.step,
            )
    ...
    def train_seg(self, true_img, true_seg, log_label):
        ...
        if (
            self.rank == 0
            and (self.step - self.step % 2) % self.args.log_train_loss_interval == 0
        ):
            self.writer.add_scalar(f"{log_label}_loss", loss, self.step)

        if (
            self.rank == 0
            and (self.step - self.step % 2) % self.args.log_train_images_interval == 0
        ):
            self.writer.add_image(
                f"{log_label}_pred_seg",
                make_grid(
                    pred_seg.flatten(0, 1).float().sigmoid(),
                    nrow=self.args.seq_length_lr,
                ),
                self.step,
            )
            self.writer.add_image(
                f"{log_label}_true_seg",
                make_grid(true_seg.flatten(0, 1), nrow=self.args.seq_length_lr),
                self.step,
            )
            self.writer.add_image(
                f"{log_label}_true_img",
                make_grid(true_img.flatten(0, 1), nrow=self.args.seq_length_lr),
                self.step,
            )
            ...
            with torch.no_grad():
                with autocast(enabled=not self.args.disable_mixed_precision):
                    for true_fgr, true_pha, true_bgr in tqdm(
                        ...
                        total_loss += (
                            matting_loss(pred_fgr, pred_pha, true_fgr, true_pha)[
                                "total"
                            ].item()
                            * batch_size
                        )
    ...
    def random_crop(self, *imgs):
        ...
        for img in imgs:
            ...
            img = img.flatten(0, 1)
            img = F.interpolate(
                img, (max(h, w), max(h, w)), mode="bilinear", align_corners=False
            )
    ...
    def save(self):
        if self.rank == 0:
            ...
            torch.save(
                ...
                os.path.join(self.args.checkpoint_dir, f"epoch-{self.epoch}.pth"),
            )
