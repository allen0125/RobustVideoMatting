...
for dataset in project.datasets:
    for item in tqdm(dataset):
        ...
        if img.size[0] > max_size or img.size[1] > max_size:
            ...
            msk = msk.resize(
                (int(msk.size[0] * scale), int(msk.size[1] * scale)), Image.NEAREST
            )

        img.save(
            os.path.join(output_path, "train", "src", item.replace(".png", ".jpg"))
        )
        msk.save(
            os.path.join(output_path, "train", "msk", item.replace(".png", ".jpg"))
        )
