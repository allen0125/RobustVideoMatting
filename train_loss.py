...
def matting_loss(pred_fgr, pred_pha, true_fgr, true_pha):
    ...
    loss["pha_laplacian"] = laplacian_loss(
        pred_pha.flatten(0, 1), true_pha.flatten(0, 1)
    )
    loss["pha_coherence"] = (
        F.mse_loss(
            pred_pha[:, 1:] - pred_pha[:, :-1], true_pha[:, 1:] - true_pha[:, :-1]
        )
        * 5
    )
    # Foreground losses
    ...
    loss["fgr_coherence"] = (
        F.mse_loss(
            pred_fgr[:, 1:] - pred_fgr[:, :-1], true_fgr[:, 1:] - true_fgr[:, :-1]
        )
        * 5
    )
    # Total
